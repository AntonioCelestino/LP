                    ...
                    <td>Formulário: (Nome do Aluno | Seleção Modalidade | Código Formulário): 
                        <select name="optFormulario" <c:if test="${operacao != 'Incluir'}"> disabled</c:if>>
                            ...
                            <c:forEach items="${formularios}" var="formulario">
                                <option value="${formulario.codFormulario}" <c:if test="${doenca.formulario.codFormulario == formulario.codFormulario}"> selected</c:if>>${formulario.aluno.usuario.nome} | ${formulario.selecao.numeroEdital} | ${formulario.selecao.modalidade.nome} | ${formulario.codFormulario}</option>  
                            </c:forEach>
                        </select>
